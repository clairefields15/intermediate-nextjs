import { getEventsForDashboard } from '@/utils/events'
import { getRsvpsForDashboard } from '@/utils/rsvps'
import { getCurrentUser } from '@/utils/users'
import { Chip, ChipProps } from '@nextui-org/react'
import Link from 'next/link'

const statusColors: { [name: string]: ChipProps['color'] } = {
	draft: 'warning',
	live: 'success',
	started: 'primary',
	ended: 'warning',
	canceled: 'danger',
}
const RsvpsSlot = async () => {
	const user = await getCurrentUser()
	const data = await getRsvpsForDashboard(user.id)

	return (
		<div className="w-full h-full p-4 flex justify-center">
			<div className="w-full">
				<h2 className="text-center text-xl">{`RSVPs`}</h2>
				<div className="rounded-md border border-default-100 my-8">
					{data.map(({ rsvps, events, attendees }, idx) => {
						if (!rsvps) return <div key={idx}>No rsvps</div>
						if (!events) return <div key={idx}>No events</div>
						return (
							<div
								key={rsvps.id}
								className="border-b border-default-100 p-2 flex gap-2"
							>
								<span>{attendees.name}</span>
								<span>
									<Chip
										size="sm"
										color={statusColors[rsvps?.status]}
									>
										{rsvps?.status}
									</Chip>
								</span>
								<span>
									<Link
										href={`/dashboard/events/${events.id}`}
									>
										<Chip size="sm" variant="faded">
											{events?.name}
										</Chip>
									</Link>
								</span>
							</div>
						)
					})}
				</div>
			</div>
		</div>
	)
}

export default RsvpsSlot
